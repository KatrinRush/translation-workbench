...
cd ~/translation-workbench   # заміни на свій реальний шлях до репо
git checkout main
git pull
git checkout -b fix/epub-html-entities

cat > backend/parsers/epub.py << 'PYEOF'
...
git diff backend/parsers/epub.py
pytest
git add backend/parsers/epub.py
git commit -m "fix(epub): normalize HTML entities before XML parse; split chapters by real markers, not by spine file"
git push -u origin fix/epub-html-entities
